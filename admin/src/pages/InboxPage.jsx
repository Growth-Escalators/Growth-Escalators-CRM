    ...
    <div className={`flex ${isOut ? 'justify-end' : 'justify-start'} mb-2`}>
      <div className={`max-w-xs lg:max-w-md xl:max-w-lg rounded-2xl px-4 py-2 ${
        ...
        <div className={`flex items-center gap-1 mt-1 ${isOut ? 'justify-end' : 'justify-start'}`}>
          <span className={`text-xs ${isOut ? 'text-success-500/10' : 'text-neutral-500'}`}>
            {timeAgo(msg.sentAt)}
          </span>
...
function TemplateSelector({ templates, onSelect, onClose }) {
  return (
    <div className="absolute bottom-full left-0 mb-2 bg-white rounded-xl border border-neutral-200 shadow-lg w-72 max-h-60 overflow-y-auto z-10">
      ...
      {templates.length === 0 && <p className="p-3 text-xs text-neutral-500 text-center">No approved templates</p>}
      {templates.map(t => (
        <button
          key={t.id}
        ...
        >
          ...
          <p className="text-xs text-neutral-500">{t.category} · {t.language}</p>
        </button>
      ))}
    </div>
  );
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && filtered.length === 0 && (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-neutral-500 mt-1">Messages will appear here when WhatsApp is connected</p>
            </div>
          )}
          {filtered.map(conv => {
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <p className="text-xs text-neutral-500 flex-shrink-0 ml-2">{timeAgo(conv.lastMessageAt)}</p>
                  </div>
                  {conv.companyName && (
                    <p className="text-xs text-neutral-500 mb-0.5 truncate">{conv.companyName}</p>
                  )}
                  <div className="flex items-center justify-between">
                    <p className={`text-xs truncate ${unread > 0 ? 'text-neutral-700' : 'text-neutral-500'}`}>
        ...
        {!selectedConv ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-neutral-500 text-sm mt-1">Choose a contact to see your WhatsApp messages</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 bg-neutral-50">
              {msgLoading && <p className="text-center text-sm text-neutral-500 py-8">Loading messages…</p>}
              {!msgLoading && messages.length === 0 && (
                <p className="text-center text-sm text-neutral-500 py-8">No messages yet. Send the first message!</p>
              )}
              {messages.map(msg => (
