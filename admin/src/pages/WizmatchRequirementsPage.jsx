      ...
      <div className="bg-white w-[560px] max-w-[95vw] h-full overflow-y-auto shadow-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            {mode === 'paste' ? (
              <textarea aria-label="Job description text" value={jdText} onChange={e => { setJdText(e.target.value); setParseFeedback(null); }} rows={5} placeholder="Paste the client's job requirement here…" className="input w-full resize-y" />
            ) : (
...
function RequirementOperations({ requirement }) {
  ...
  return <section className="card p-4 border-primary-200 bg-primary-50/30 space-y-4">
    ...
    <div><div className="text-[11px] uppercase font-semibold text-neutral-500 mb-1">Assigned team</div>{data.assignments.filter(a=>a.active).map(a=><div key={a.id} className="text-[12px] mb-1 flex justify-between"><span><b>{a.name}</b> · {a.role.replaceAll('_',' ')}</span><button className="text-danger-600" disabled={busy} onClick={()=>run(()=>apiFetch(`/api/wizmatch/requirements/${requirement.id}/assignments/${a.id}`,{method:'DELETE'}),'Assignment removed')}>Remove</button></div>)}<div className="grid grid-cols-[1fr_1fr_auto] gap-2 mt-2"><select aria-label="Team member to assign" className="input" value={userId} onChange={e=>setUserId(e.target.value)}>{users.map(u=><option key={u.id} value={u.id}>{u.name} · {u.role}</option>)}</select><select aria-label="Assignment role" className="input" value={assignmentRole} onChange={e=>setAssignmentRole(e.target.value)}><option value="account_owner">Account owner</option><option value="delivery_owner">Delivery owner</option><option value="recruiter">Recruiter</option></select><button className="btn-standard btn-compact" disabled={busy||!userId} onClick={()=>run(()=>apiFetch(`/api/wizmatch/requirements/${requirement.id}/assignments`,{method:'POST',body:JSON.stringify({userId,role:assignmentRole})}),'Team member assigned')}>Assign</button></div></div>
    <div><div className="text-[11px] uppercase font-semibold text-neutral-500 mb-1">Dated next action and SLA</div><input aria-label="Next action" className="input w-full mb-2" placeholder="Example: Call Priya for Java shortlist feedback" value={nextAction} onChange={e=>setNextAction(e.target.value)}/><div className="grid grid-cols-2 gap-2"><label className="text-[11px]">Next action due<input type="datetime-local" className="input w-full mt-1" value={dueAt} onChange={e=>setDueAt(e.target.value)}/></label><label className="text-[11px]">Requirement SLA due<input type="datetime-local" className="input w-full mt-1" value={slaAt} onChange={e=>setSlaAt(e.target.value)}/></label></div><button className="btn-standard btn-compact mt-2" disabled={busy||!nextAction.trim()||!dueAt} onClick={()=>run(()=>apiFetch(`/api/wizmatch/requirements/${requirement.id}/next-action`,{method:'POST',body:JSON.stringify({nextAction,nextActionDueAt:new Date(dueAt).toISOString(),slaDueAt:slaAt?new Date(slaAt).toISOString():undefined})}),'Next action and linked task created')}>Save next action</button></div>
    <div><div className="text-[11px] uppercase font-semibold text-neutral-500 mb-1">Operating stage</div><div className="flex gap-2"><select aria-label="Operating stage" className="input flex-1" value={targetStage} onChange={e=>{setTargetStage(e.target.value);setClosureReason('');}}>{OPERATING_STAGES.map(s=><option key={s} value={s}>{s.replaceAll('_',' ')}</option>)}</select><button className="btn-primary btn-compact" disabled={busy||targetStage===data.requirement.stage||(['closed_lost','cancelled'].includes(targetStage)&&!closureReason.trim())} onClick={()=>run(()=>apiFetch(`/api/wizmatch/requirements/${requirement.id}/transition`,{method:'POST',body:JSON.stringify({stage:targetStage,closureReason:['closed_lost','cancelled'].includes(targetStage)?closureReason.trim():undefined})}),'Stage updated')}>Move stage</button></div>{['closed_lost','cancelled'].includes(targetStage)&&<input aria-label="Closure reason" className="input w-full mt-2" placeholder="Closure reason (required)" value={closureReason} onChange={e=>setClosureReason(e.target.value)}/>}<p className="text-[11px] text-neutral-500 mt-1">Acceptance is blocked until primary source, account owner, recruiter, SLA and dated next action are all present.</p></div>
