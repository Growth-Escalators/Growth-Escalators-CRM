...
export default function WizmatchTalentMatchingPage() {
  ...
  return <div className="p-6 space-y-5">
    ...
    {loading ? <div className="card p-8 text-center text-neutral-500">Loading recruiter decisions…</div>
      : items.length === 0 ? <div className="card p-8 text-center"><p className="text-neutral-500 font-medium">No candidate matches need your review yet.</p><p className="text-[12.5px] text-neutral-500 mt-1 mb-3">This queue fills from a requirement's "Recalculate matches". Open a requirement and recalculate to populate it.</p><button onClick={() => navigate('/wizmatch/requirements')} className="btn-primary btn-compact inline-flex items-center gap-1">Go to Requirements <ArrowRight className="w-3.5 h-3.5"/></button></div>
      ...
      {filtered.length === 0 ? <div className="card p-8 text-center lg:col-span-2 text-neutral-500">No matches match these filters{blockedCount ? ' — try clearing “Hide blocked”.' : '.'}</div> : filtered.map(item => <article className="card p-4" key={item.id}>
        <div className="flex justify-between gap-3"><div><div className="font-semibold text-neutral-900">{[item.first_name,item.last_name].filter(Boolean).join(' ')}</div><div className="text-[12px] text-neutral-500">{item.requirement_title}</div></div><div className="text-right"><div className="text-xl font-bold text-primary-700">{item.score}</div><div className="text-[10px] text-neutral-500">{item.score_version}</div></div></div>
