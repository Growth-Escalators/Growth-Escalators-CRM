...
function trashDaysRemaining(dateStr) {
  ...
  return Math.max(0, TRASH_RESTORE_DAYS - Math.max(0, daysAgo(dateStr)));
}
...
// DealCard
// ---------------------------------------------------------------------------
function DealCard({ deal, index, onClick, onArchive, onUnarchive, onDelete, onRestore, selected = false, onToggleSelect, selectionMode = false }) {
        ...
        >
                    ...
                    {isDeleted ? (
                      ...
                      <>
                        {isArchived ? (
                          ...
                          <button onClick={() => { onArchive(); setMenuOpen(false); }} className="w-full text-left px-3 py-2 text-sm text-neutral-700 hover:bg-neutral-50">Archive</button>
                        )}
                        <button
          ...
          {isDeleted && (
            <div className="mt-2 mb-1 flex items-center gap-1.5 text-[10px] font-medium text-danger-600">
              ...
              {restoreExpired ? 'Restore window expired' : `${trashDaysRemaining(deal.metadata?.deletedAt)}d left in restore window`}
            </div>
          )}
...
export default function PipelinePage() {
  ...
  // Clear bulk selection when switching pipelines or toggling archived/trash views
  ...
  function removeDealsLocally(ids) {
    ...
    setKanbanStages((prev) => prev.map((s) => {
      const deals = (s.deals ?? []).filter((d) => !idSet.has(d.id));
      return withRecalculatedValue(s, deals);
    }));
  }
  ...
  const selectedTrashHasExpired = showTrash && Array.from(selectedIds).some((id) => isTrashRestoreExpired(findDeal(id)?.metadata?.deletedAt));
      ...
      <main className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
        ...
        <div className="bg-white border-b px-6 py-4 shrink-0">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <button
              onClick={() => { setShowTrash((v) => !v); setShowArchived(false); }}
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            <button type="button" onClick={() => scrollBoard(-1)} className="p-1.5 border border-neutral-200 rounded-lg text-neutral-500 hover:bg-neutral-50" aria-label="Scroll pipeline left">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button type="button" onClick={() => scrollBoard(1)} className="p-1.5 border border-neutral-200 rounded-lg text-neutral-500 hover:bg-neutral-50" aria-label="Scroll pipeline right">
        ...
        {loadError && !loading ? (
          ...
          <div className="flex flex-1 min-h-0 gap-4 px-4 py-4 overflow-x-auto overflow-y-hidden">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className="min-w-[280px] w-[280px] h-full shrink-0 rounded-xl border border-neutral-200 bg-neutral-100 animate-pulse"/>
            ))}
          </div>
        ) : (
          <DragDropContext onDragEnd={onDragEnd}>
            <div ref={boardRef} className="flex flex-1 min-h-0 gap-4 px-4 py-4 overflow-x-auto overflow-y-hidden overscroll-x-contain scroll-smooth">
              {kanbanStages.map((stageData, stageIndex) => {
                ...
                const visibleStageValue = stageDeals.reduce((sum, deal) => sum + Number(deal.dealValue || 0), 0);
                return (
                  <div key={stageData.stageId || stageData.stageName} className={`min-w-[85vw] w-[85vw] md:min-w-[280px] md:w-[280px] shrink-0 self-stretch min-h-0 flex flex-col rounded-xl border ${light}`}>
                    {/* Column header */}
                    <div
                      className="rounded-t-xl px-3 py-2.5 shrink-0"
                    ...
                    >
                      ...
                      {visibleStageValue > 0 && (
                        <p className="text-white/75 text-[10px] font-medium">{fmtInr(visibleStageValue)}</p>
                      )}
                    </div>
                      ...
                      {(provided, snapshot) => (
                        ...
                        >
                          ...
                          {stageDeals.length === 0 && !snapshot.isDraggingOver && (
                            <p className="text-center text-xs text-neutral-600 py-2">{showTrash ? 'No deleted deals' : 'Empty'}</p>
                          )}
                        </div>
                      )}
                    </Droppable>
