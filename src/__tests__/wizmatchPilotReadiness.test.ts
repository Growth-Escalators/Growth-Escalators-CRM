...
describe('assessWizmatchPilotReadiness — safety properties', () => {
  ...
  it('warns when a migration ABOVE the authorised high-water mark appears', () => {
    ...
    // (now 0049, adding the seo_api_usage spend ledger) must not turn it
    ...
    const probe = join(migrationsDir, '0050__readiness_sentinel_probe.sql');
    ...
    try {
      ...
      expect(migrationFinding?.message).toContain('0050__readiness_sentinel_probe.sql');
    } finally {
