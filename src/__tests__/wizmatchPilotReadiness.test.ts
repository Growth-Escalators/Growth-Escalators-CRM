...
describe('assessWizmatchPilotReadiness — safety properties', () => {
  ...
  it('warns when a migration ABOVE the authorised high-water mark appears', () => {
    ...
    // (now 0051, user_invites — invite-by-email) must not turn it off. The
    ...
    const probe = join(migrationsDir, '0052__readiness_sentinel_probe.sql');
    ...
    try {
      ...
      expect(migrationFinding?.message).toContain('0052__readiness_sentinel_probe.sql');
    } finally {
