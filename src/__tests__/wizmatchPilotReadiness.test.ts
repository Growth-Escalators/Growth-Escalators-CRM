...
describe('assessWizmatchPilotReadiness — safety properties', () => {
  ...
  it('warns when a migration ABOVE the authorised high-water mark appears', () => {
    ...
    // (now 0046, user_invites — invite-by-email) must not turn it off. The
    // probe therefore always sits ONE ABOVE the current mark, and exercises
    // the real filesystem branch rather than a stub. If you bump the mark
    // again, bump this probe with it — a probe at or below the mark would
    // make this test vacuous.
    const migrationsDir = join(repoRoot, 'src', 'db', 'migrations');
    const probe = join(migrationsDir, '0047__readiness_sentinel_probe.sql');
    ...
    try {
      ...
      expect(migrationFinding?.message).toContain('0047__readiness_sentinel_probe.sql');
    } finally {
