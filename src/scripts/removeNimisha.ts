...
async function removeNimisha() {
  ...
  console.log(`[remove-nimisha] Deactivated: ${(deactivateResult as unknown as { rowCount: number }).rowCount} row(s) — is_active=false + token_version=-1: existing sessions invalidated AND re-login blocked`);
