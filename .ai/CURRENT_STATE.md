...
- CRM + API live on Railway (`web` service); background jobs on the `worker` service.
...
- Worker service serves only a health probe, not the API → worker crons must call
  `WIZMATCH_API_BASE_URL` (public `web` URL), not `localhost`.

## How to rebuild context fast

`git pull origin main` → read `.ai/CURRENT_TASK.md` → `npm run ai:brief` → read `AGENTS.md`.
