...
| **QA-16** | Additional IDOR / tenant-tampering candidates reported by the security lane: bookings detail, `messages`/`sequences` taking `tenantId` from `req.body`, growthOS GET routes with no permission check and no `tenant_id` column. | HIGH (if confirmed) | **PLAUSIBLE, not verified by me.** Reported by a lane and not independently reproduced in the time available. Listed as the **top follow-up** — see roadmap. I do not assert these as confirmed defects. **The growthOS portion is MOOT as of 2026-08-05** — Growth OS was deleted entirely; the bookings/messages/sequences candidates remain open. |
