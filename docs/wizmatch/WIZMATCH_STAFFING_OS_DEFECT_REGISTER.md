...
| D-18 | P2 | 0/4 | `staging_verified` | Results-first source controls are live on `1112e47`; commits `c293b88`/`142eb51` add SearchAPI.io, real TheirStack preview/account evidence, shared caps, hiring-team parsing, seen-ID exclusion and transient retry. Live staging passed TheirStack, ATS, POC and requirement-first X-Ray with no persisted duplicates or guessed channels. | Deploy the reviewed provider commits, activate production sequentially and monitor the capped two-user pilot. |
...
| D-35 | P1 | 4 | `staging_verified` | Commit `1112e47` provides independent controls and audit. The provider release candidate passed live staging with 29 distinct TheirStack signals, an ATS refresh, six POC candidates and 10 requirement-linked unreviewed X-Ray leads; responsive UI and provider failures remained isolated. | Push the exact candidate, activate source/TheirStack/ATS/POC, then enable manual X-Ray only for a genuine reviewed requirement. |
